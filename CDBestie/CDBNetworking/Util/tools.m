...
+ (NSString*)timeLabelTextOfTime:(NSTimeInterval)time
{
    ...
    //NSLog(@"%@",text);
    //NSLog(@"%d",((int)[[NSDate date] timeIntervalSince1970])/86400);
    //NSLog(@"%d",(int)time/86400);
    //int dateAgo =((int)[[NSDate date] timeIntervalSince1970])/86400 - (int)time/86400 ;
    
    
    //NSDate *yesterday = [NSDate dateWithTimeIntervalSinceNow: -(24 * 60 * 60)];
    //NSLog(@"yesterday:%@",yesterday);
    
    //通过比较天数来确定 今天 昨天 前天(lewcok)
...
NSDateComponents *comps = [[NSDateComponents alloc] init];
    ...
    comps2 = [calendar components:unitFlags fromDate:date];
    int week = [comps weekday];
    int year=[comps year];
    int month = [comps month];
    int day = [comps day];
    int day2 = [comps2 day];
    int dateAgo = day-day2;
