...
+ (instancetype)sharedManager {
    ...
    dispatch_once(&onceToken, ^{
        ...
        //替换websocket地址(lewcok)
        NSUserDefaults *defaults =[NSUserDefaults standardUserDefaults];
        _sharedManager.baseURL = [NSURL URLWithString:[defaults objectForKey:@"SERVER_URL"]];
               ...
               failure:(MLNetworkingFailureBlcok)failure
{
    ...
    if ([action isEqualToString:KeyChain_Laixin_post_add]) {
        ...
        int count = (int)[USER_DEFAULT integerForKey:KeyChain_Laixin_post_add_count];
        count += 1;
    ...
    if ([action isEqualToString:KeyChain_Laixin_session_start2]) {
        //session.start 这里做标记处理
...
- (SRWebSocket*)webSocket
{
    ...
    if (!_webSocket) {
        ...
        //替换websocket地址(lewcok)
        NSUserDefaults *defaults =[NSUserDefaults standardUserDefaults];
        _webSocket = [[SRWebSocket alloc] initWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[defaults objectForKey:@"SERVER_URL"]]]];
